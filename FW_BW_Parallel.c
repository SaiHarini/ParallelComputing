...
printf("----------1");
	  ...
	  #pragma omp parallel for default(shared) schedule(dynamic,1) private(i,j,k) shared(num,d_count,p_count,D,P,V,r_count,R) ordered {     
            for(i=0;i<num;i++){
...
printf("----------1"); 
	  ...
	  #pragma omp parallel for default(shared) schedule(dynamic,1) private(i,j) shared(d_count,p_count,D,P,S,s_count) ordered collapse(2) {             
            for(i=0;i<d_count;i++){
            	for(j=0;j<p_count;j++){
            		/*if(j==i){
            			for(k=0;k<s_count;k++){
            ...
            for(i=0;i<s_count;i++){
             ...
             #pragma omp parallel for default(shared) schedule(dynamic,1) private(i,k) shared(d_count,s_count,D,S,ds_count,D_S) ordered {  
            for(i=0;i<d_count;i++){
            for(k=0;k<s_count;k++){
            	if(D[i]==S[k])
            		break;
