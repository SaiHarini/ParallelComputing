...
while(n!=0){
	...
	while(flag==1){
		...
		#pragma omp parallel for default(shared) schedule(dynamic,1) private(u,j,v) shared(graph,n,flag,colours) ordered collapse(2) {
		//printf("Thread id=%d\n",omp_get_thread_num());
...
#pragma omp parallel for default(shared) schedule(dynamic,1) private(c,i,j,temp,u,v,VC,count,SCV,scv_count) shared(V,num,flag,graph,colours,visited) ordered collapse(2) {
